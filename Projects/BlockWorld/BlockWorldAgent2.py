...
class Solve:
    ...
    def solve(self, initial_arrangement, goal_arrangement):
        initialstate= (tuple(var) for var in initial_arrangement if var)
        goalstate= (tuple(var) for var in goal_arrangement if var)
        # goalstate=(("F", "D", "C", "I", "G", "A"), ("B", "E", "H"))
        # initialstate=(("A", "B", "C"), ("D", "E", "F"), ("G", "H", "I"))
        problem3 = BlocksWorldH2(initialstate, goalstate)
        print("problem format ", problem3)
        ...
        for iterator in range(1, len(astar_path)):
            ...
            if action[1] != ' ':
                loadtoUEFA.append(tuple((state[action[0]][-1], state[action[1]][-1])))
            else:
                loadtoUEFA.append(tuple((state[action[0]][-1], "Table")))
      

        print("Found in {0} \n\n".format(end_time_astar - start_time_astar))  # Execution time for A* search

        # while(current.action!=None):
        #     print(current.action)
        #     path.insert(0,tuple(current.action))
...
class BlockWorldAgent:
    def __init__(self, initial, goal):
        self.initial = (tuple(var) for var in initial if var)
        self.goal = (tuple(var) for var in goal if var)
    def result(self, state, action):
    ...
    def solve(self, initial_arrangement, goal_arrangement):
        ...
        # pass
        # solvePuzzle  = Solve()
        # return solvePuzzle.solve(initial_arrangement, goal_arrangement)
        # initialstate= (tuple(var) for var in initial_arrangement if var)
        # goalstate= (tuple(var) for var in goal_arrangement if var)
        # goalstate=(("F", "D", "C", "I", "G", "A"), ("B", "E", "H"))
        # initialstate=(("A", "B", "C"), ("D", "E", "F"), ("G", "H", "I"))
